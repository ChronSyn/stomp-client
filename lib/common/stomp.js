...
exports.StompClient = exports.symbols = undefined;
...
const symbols = exports.symbols = new Map().set('host', Symbol('host')).set('port', Symbol('port')).set('timeout', Symbol('timeout')).set('channel', Symbol('channel')).set('subscriptions', Symbol('subscriptions'));
 ...
 */
class StompClient extends _events2.default {
  /**
   * @constructor
  ...
  initialise() {
    return new Promise((resolve, reject) => {
      if (!this.isInitialised) {
        const servers = new _stompit2.default.ConnectFailover([this.serverConfig]);
        ...
        this[symbols.get('channel')] = new _stompit2.default.Channel(servers, { alwaysConnected: true });
    ...
    return new Promise((resolve, reject) => {
      if (this.isInitialised) {
        this[symbols.get('channel')].subscribe(queue, (err, message) => {
          ...
          message.pipe(_zlib2.default.createGunzip()).on('error', err => {
            this.emit('error', err);
          }).on('data', chunk => {
